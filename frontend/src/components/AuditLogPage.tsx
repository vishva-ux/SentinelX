...
export const AuditLogPage: React.FC = () => {
  ...
  const actions = ['All', 'LOGIN_SUCCESS', 'LOGIN_FAILURE', 'ROLE_ASSIGNED', 'CONFIG_CHANGE'];
  const filtered = AUDIT_LOGS.filter(l => filter === 'All' || l.action === filter);
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex gap-1 mb-4">
        {actions.map(a => (
          <button key={a}
            onClick={() => setFilter(a)}
            className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-colors ${filter === a ? 'bg-white border border-gray-300 text-gray-900 shadow-xs' : 'text-gray-500 hover:text-gray-800'}`}
          >{a === 'All' ? 'All Actions' : ACTION_LABELS[a]}</button>
        ))}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <table className="w-full text-xs">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              {['Time', 'Action', 'Actor', 'Resource / Target', 'Source IP', 'Outcome'].map(h => (
          ...
          <tbody className="divide-y divide-gray-100">
            {filtered.map(log => (
              <tr key={log.id} className={`hover:bg-gray-50 transition-colors ${log.outcome === 'Failed' ? 'bg-red-50/30' : ''}`}>
                ...
                <td className="py-3 px-4 font-mono font-medium text-gray-800">{log.actor}</td>
                ...
                <td className="py-3 px-4">
                  <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                    log.outcome === 'Success' ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-red-50 text-red-700 border border-red-200'
                  }`}>{log.outcome}</span>
                </td>
              </tr>
            ))}
